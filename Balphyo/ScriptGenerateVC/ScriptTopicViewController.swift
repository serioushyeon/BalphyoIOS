//
//  ScriptTopicViewController.swift
//  Balphyo
//
...
class ScriptTopicViewController: UIViewController {
    
    let instructionLabel = UILabel().then {
        $0.text = "발표의 주제를 알려주세요"
        $0.textColor = .Black
    ...
    @objc func nextButtonTapped() {
        guard let scriptTopic = scriptTopicTextfield.customTextField.text, !scriptTopic.isEmpty else {
            let alert = UIAlertController(title: "주제 입력", message: "대본 주제를 입력해 주세요.", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "확인", style: .default, handler: nil))
            present(alert, animated: true, completion: nil)
            return
        }
        GenerateScriptManager.shard.topic = scriptTopic
        ...
        let nextVC = ScriptSubTopicViewController()
